...
export AWS_USER_POOLS_ID=`aws cloudformation describe-stacks --stack-name $STACK_NAME --query "Stacks[0].Outputs[?OutputKey=='CognitoID'].OutputValue" --output text`
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  sed -i '' -e 's/AWS_USER_POOLS_ID/'"$AWS_USER_POOLS_ID"'/g' www/src/config.js
